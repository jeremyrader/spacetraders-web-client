      ...
      {marketData ? (
        <main className="flex min-h-screen flex-col p-24">
          <h1 className="text-xl text-center mb-10">
            Marketplace {marketData.symbol}
          </h1>
          <ul className="menu menu-vertical lg:menu-horizontal bg-base-200 rounded-box mb-8">
            {marketData.exchange && (
              <li>
                <a
                  className={`${selectedView === 'Exchange' ? 'bg-base-300' : 'bg-base-200'}`}
                  onClick={() => {
                    setSelectedView('Exchange');
                  }}
                >
                  Exchange
                </a>
            ...
            {marketData.imports && (
              <li>
                <a
                  className={`${selectedView === 'Imports' ? 'bg-base-300' : 'bg-base-200'}`}
                  onClick={() => {
                    setSelectedView('Imports');
                  }}
                >
                  Imports
                </a>
            ...
            {marketData.exports && (
              <li>
                <a
                  className={`${selectedView === 'Exports' ? 'bg-base-300' : 'bg-base-200'}`}
                  onClick={() => {
            ...
            {marketData.tradeGoods && (
              <li>
                <a
                  className={`${selectedView === 'Trade Goods' ? 'bg-base-300' : 'bg-base-200'}`}
                  onClick={() => {
                    setSelectedView('Trade Goods');
                  }}
                >
                  Trade Goods
                </a>
            ...
            {marketData.transactions && (
              <li>
                <a
                  className={`${selectedView === 'Transactions' ? 'bg-base-300' : 'bg-base-200'}`}
                  onClick={() => {
          ...
          {marketData.exchange.length > 0 && selectedView == 'Exchange' && (
            <table className="table">
              ...
              <tbody>
                {marketData?.exchange &&
                  marketData.exchange.map(
                    (exchange: ICommodity, index: number) => {
                      return (
                        <tr key={index}>
                          <th>{exchange.name}</th>
          ...
          {marketData.imports.length > 0 && selectedView == 'Imports' && (
            <table className="table">
              ...
              <tbody>
                {marketData?.imports &&
                  marketData.imports.map(
                    (tradeImport: ICommodity, index: number) => {
                      return (
                        <tr key={index}>
          ...
          {marketData.exports.length > 0 && selectedView == 'Exports' && (
            <table className="table">
              ...
              <tbody>
                {marketData?.exports &&
                  marketData?.exports.map(
                    (tradeExport: ICommodity, index: number) => {
                      return (
                        <tr key={index}>
          ...
          {marketData.tradeGoods &&
            marketData.tradeGoods.length > 0 &&
            selectedView == 'Trade Goods' && (
              <table className="table">
                <thead>
                  <tr>
                    <th>Symbol</th>
                ...
                <tbody>
                  {marketData.tradeGoods.map(
                    (tradeGood: ITradeGood, index: number) => {
                      return (
                        <tr key={index}>
