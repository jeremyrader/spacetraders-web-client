...
const Waypoint = ({
  ...
  const isObscured =
    !!selectedWaypoint && selectedWaypoint.symbol !== waypoint.symbol;
  const isSelected =
    selectedWaypoint != null && selectedWaypoint.symbol === waypoint.symbol;
  const isHighlighted = waypoint.traits
    ? getIsHighlighted(waypoint.traits, selectedTrait)
    : false;
      ...
      {orbits && x && y ? (
        <Group>
          <Circle
            ...
            fillLinearGradientColorStops={[0, color1, 0.7, color2, 1, 'white']}
            ...
            onClick={() => {
              onWaypointClick(waypoint, orbitalRef);
            }}
