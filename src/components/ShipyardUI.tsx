      ...
      {shipyardData ? (
        <main className="flex min-h-screen flex-col p-24">
          <h1 className="text-xl text-center">
            Shipyard {shipyardData.symbol}
          </h1>
          <p className="text-lg text-center text-neutral-500 mb-10">
            Modifications Fee <span>{shipyardData.modificationsFee}</span>
          </p>

          {!shipyardData.ships ? (
            <div className="text-center">
              This shipyard sells the following ship types:
              {shipyardData.shipTypes.map((shipType, index) => (
                <p key={index}>
                  {shipTypeNameMapping[shipType.type as TShipType]}
                </p>
              ))}
            </div>
          ) : null}
          {shipyardData.ships && shipyardData.transactions ? (
            <ul className="menu menu-vertical lg:menu-horizontal bg-base-200 rounded-box mb-8">
              <li>
                <a
                  className={`${selectedView === 'Ships' ? 'bg-base-300' : 'bg-base-200'}`}
                  onClick={() => {
                    setSelectedView('Ships');
                  }}
                >
                  Ships
                </a>
                    ...
                    return (
                      <tr key={index}>
                        ...
                        <td>
                          {
                            shipTypeNameMapping[
                              transaction.shipType as TShipType
                            ]
                          }
                        </td>
